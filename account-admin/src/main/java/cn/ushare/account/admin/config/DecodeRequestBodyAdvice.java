...
//            log.info("【接受的请求数据】", builderString);
            try {
                String decodeString = securityService.decrypt(builderString);
                log.info("【解密后的请求数据】", decodeString);
                //把数据放到我们封装的对象中
                return new MyHttpInputMessage(inputMessage.getHeaders(), new ByteArrayInputStream(decodeString.trim().getBytes("UTF-8")));
            } catch (Exception e) {
                e.printStackTrace();
                return null;
            }
