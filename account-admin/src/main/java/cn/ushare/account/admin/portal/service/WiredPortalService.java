...
@Service
@Slf4j
public class WiredPortalService {
    ...
    public BaseResult getAcParam(HttpServletRequest request) throws Exception {
        ...
        UrlParameter urlParameter = urlParameterService.getById(1);
        //有线ac不回传AC地址，故从ac里面取出唯一的ac
        DeviceBrand brand = brandService.getOne(new QueryWrapper<DeviceBrand>().eq("code", "wired"));
        Ac ac = acService.getOne(new QueryWrapper<Ac>()
                .eq("is_valid", 1)
                .eq("brand_id", brand.getId()));
        String acIp = ac.getIp();

        String apIp = request.getParameter(urlParameter.getApIp());
        String apMac = request.getParameter(urlParameter.getApMac());
        if (StringUtil.isNotBlank(apMac)) {// 去掉“-”，转换大写
            apMac = apMac.replace("-", "");
            apMac = apMac.toUpperCase();
        }
        String userVisitUrl = request.getParameter(urlParameter.getUrl());
        ...
        String userIp = request.getParameter(urlParameter.getUserIp());
        String userMac = request.getParameter(urlParameter.getUserMac());
        if (StringUtil.isNotBlank(userMac)) {
            userMac = PortalUtil.MacFormat1(userMac);
        }
        String basName = request.getParameter(urlParameter.getBasName());
        ...
        if (acResult.getReturnCode().equals("0")) {// 返回重定向地址：错误提示页面
            acResult = acService.getInfoByAcName(basName);
            if (acResult.getReturnCode().equals("0")) {
                acResult = acService.getInfo4Wired();
            }
        }

        ac = (Ac) acResult.getData();

        String ssid = ac.getIp().replace(".", "-");
        // 保存ssid参数
    ...
    public BaseResult login(AuthParam authParam) throws Exception {
        ...
        if(StringUtils.isEmpty(authParam.getAcIp()) || "null".equals(authParam.getAcIp())){
            Ac ac = acService.getInfo4Wired().data;
            if (ac == null) {
                return new BaseResult("0", "该AC设备未登记", null);
            }
            log.error("acIp===" + ac.getIp());
        ...
        //有线认证缺少SSID
        authParam.setSsid(authParam.getAcIp().replace(".","-"));
        // 浏览器类型
        String userAgent = request.getHeader("User-Agent");
        Integer terminalType = BrowseTypeUtil.getTerminalType(userAgent);
...
//        authParam.setAuthMethod(Constant.AuthMethod.ACCOUNT_AUTH);// 借用一键登录方式
    ...
    public BaseResult logoutByRadius(AuthLogoutParam authLogoutParam) {
        ...
        BaseResult result = radiusCoaService.requestCoa(timeout, ac.getIp(), userIp, null,
        ...
        if("-2".equals(result.getReturnCode())){
            result = radiusCoaService.requestCoa(timeout, ac.getIp(), userIp,
                    authRecord.getMac(), sharedSecret, acctSessionId);
    ...
    public BaseResult logout(AuthLogoutParam authLogoutParam) {
        ...
        // 需使用参数中的userIp，不能用authParam中记录的userIp，因为二维码认证时，
        // 用户可能使用任意手机浏览器扫码，没有经过ac重定向，没有用户的MAC地址
        String userIp = authLogoutParam.getUserIp();
        String userMac = authLogoutParam.getUserMac();
        ...
        return portalApi.logout(ac.getIp(), basPort, timeout, serialNo,
