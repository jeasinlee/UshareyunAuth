...
import cn.ushare.account.admin.portal.service.CiscoPortalService;
 ...
 * 【优科Portal接口】
 * （1）Ac设备端配置项检查：
 *      认证页重定向地址：http://ip:port/cisco
 *      回调地址：http://ip:port/cisco/loginCallback
 * （1）认证流程：
 ...
 *         加入用户到临时登录列表（1分钟之后没有完成微信认证，则踢下线，但cisco设备不支持踢下线），
 ...
 */
@Api(tags = "CiscoPortalController", description = "CiscoPortal接口")
@RestController
@Slf4j
@RequestMapping("/cisco")
public class CiscoPortalController {
    ...
    @Autowired
    CiscoPortalService portalService;
    @Autowired
    AcService acService;
    @Autowired
    ...
    public BaseResult getAuthParam(@RequestBody String paramJson) throws Exception {
        log.debug("cisco getAuthParam method");
    ...
    public BaseResult login(@RequestBody String authParamJson) throws Exception {
        ...
        log.debug("ciscoLogin " + authParam.toString());
        if (authParam.getAuthMethod() == Constant.AuthMethod.MAC_WHITE_LIST_AUTH) {
            // mac白名单登录
            return portalService.macWhiteListLogin2(authParam);
        } else {
