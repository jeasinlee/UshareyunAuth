...
import cn.ushare.account.admin.portal.service.ArubaPortalService;
 ...
 * 【优科Portal接口】
 * （1）Ac设备端配置项检查：
 *      认证页重定向地址：http://ip:port/aruba
 *      回调地址：http://ip:port/aruba/loginCallback
 * （1）认证流程：
 ...
 *         加入用户到临时登录列表（1分钟之后没有完成微信认证，则踢下线，但aruba设备不支持踢下线），
 ...
 */
@Api(tags = "ArubaPortalController", description = "ArubaPortal接口")
@RestController
@Slf4j
@RequestMapping("/aruba")
public class ArubaPortalController {
    ...
    @Autowired
    ArubaPortalService portalService;
    @Autowired
    AcService acService;
    @Autowired
    ...
    public BaseResult getAuthParam(@RequestBody String paramJson) throws Exception {
        log.debug("aruba getAuthParam method");
    ...
    public BaseResult login(@RequestBody String authParamJson) throws Exception {
        ...
        log.debug("arubaLogin " + authParam.toString());
    ...
    public void loginCallback() throws Exception {
        log.debug("aruba loginCallback controller");
