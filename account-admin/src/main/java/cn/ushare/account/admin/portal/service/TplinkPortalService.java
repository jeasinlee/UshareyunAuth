...
@Service
@Slf4j
public class TplinkPortalService {
    ...
    public BaseResult getAcParam(HttpServletRequest request) throws Exception {
        ...
        String acIp = request.getParameter("wlanacip");
        ...
        String userVisitUrl = request.getParameter("redirecturl");
        ...
        String userIp = request.getParameter("staIp");
        String userMac = request.getParameter("staMac");
        ...
        BaseResult acResult = null;
        if(StringUtils.isNotEmpty(acIp)) {
            // 查询basIp对应的ac的认证模板
            acResult = acService.getInfoByAcIp(acIp);
            ...
            acResult = acService.getInfoByAcName(acName);
        }
        Ac ac = (Ac) acResult.getData();

        if(ac.getBrand().getCode().equals("tplink")){
            ssid = PortalUtil.MacFormat1(apMac);
        }
        // 保存ssid参数
...
//        reqData.put("authUrl", authUrl);
    ...
    public BaseResult logoutByRadius(AuthLogoutParam authLogoutParam) {
        ...
        BaseResult result = radiusCoaService.requestCoa(timeout, acIp, userIp,
                authRecord.getUserName(), sharedSecret, acctSessionId);
