...
@Service
@Slf4j
public class HuaweiPortalService {
    ...
    public BaseResult getAcParam(HttpServletRequest request) throws Exception {
        ...
        String acIp = request.getParameter("AC-IP");
        String acMac = request.getParameter("AC-MAC");
        if (StringUtil.isNotBlank(acMac)) {
            acMac = acMac.toUpperCase();
        }
        String apIp = request.getParameter("AP-IP");
        String apMac = request.getParameter("AP-MAC");
        if (StringUtil.isNotBlank(apMac)) {// 去掉“-”，转换大写
            apMac = apMac.replace("-", "");
            apMac = apMac.toUpperCase();
        }
        String userVisitUrl = request.getParameter("redirect-url");
        ...
        String userIp = request.getParameter("user-ipaddress");
        String userMac = request.getParameter("user-mac");
